...
function RoomCard({ num, roomData, unpaidPower, unpaidWater, unpaidRent, onPayClick }) {
  ...
  return (
    <Card className={cn("transition-all", expanded && "shadow-lg ring-1 ring-border")}>
      <CardContent className="p-4 sm:p-5 cursor-pointer select-none" onClick={() => setExpanded(!expanded)}>
        ...
        <div className="border-t border-border px-4 pb-4 space-y-2 pt-3">
          ...
          {unpaidRent.map(record => (
            <div key={record.id} className="flex items-center justify-between bg-red-50 dark:bg-red-900/20 rounded-xl px-3 py-2.5 gap-2">
              ...
              <button
                onClick={e => { e.stopPropagation(); onPayClick({ room: num, type: 'rent', record }); }}
          ...
          {unpaidPower.map(record => (
            <div key={record.id} className="flex items-center justify-between bg-yellow-50 dark:bg-yellow-900/20 rounded-xl px-3 py-2.5 gap-2">
              ...
              <button
                onClick={e => { e.stopPropagation(); onPayClick({ room: num, type: 'power', record }); }}
                className="shrink-0 flex items-center gap-1 px-2.5 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded-lg text-xs font-medium transition-all"
              >
                <CreditCard className="w-3 h-3" /> Πληρωμή
              </button>
          ...
          {unpaidWater.map(record => (
            <div key={record.id} className="flex items-center justify-between bg-blue-50 dark:bg-blue-900/20 rounded-xl px-3 py-2.5 gap-2">
              ...
              <button
                onClick={e => { e.stopPropagation(); onPayClick({ room: num, type: 'water', record }); }}
                className="shrink-0 flex items-center gap-1 px-2.5 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded-lg text-xs font-medium transition-all"
              >
                <CreditCard className="w-3 h-3" /> Πληρωμή
              </button>
          ...
          <div className="flex gap-2 pt-1">
            <button
              onClick={e => { e.stopPropagation(); navigate(`/room/${num}`); }}
            ...
            <button
              onClick={e => { e.stopPropagation(); navigate(`/room/${num}/status`); }}
...
export default function Home() {
  ...
  // PaymentDialog state — opened directly from Home
  const [payDialog, setPayDialog] = useState({ open: false, preloaded: null, preloadedRoom: null });
  ...
  // Open PaymentDialog directly with preloaded data
  const handlePayClick = ({ room, type, record }) => {
    setPayDialog({
      open: true,
      preloadedRoom: room,
      preloaded: { type, record },
    });
  };
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto pb-24">
      ...
      {/* PaymentDialog opens directly from Home with preloaded data */}
      <PaymentDialog
        open={payDialog.open}
        onClose={() => setPayDialog({ open: false, preloaded: null, preloadedRoom: null })}
        onSaved={() => { setPayDialog({ open: false, preloaded: null, preloadedRoom: null }); loadData(); }}
        preloaded={payDialog.preloaded}
        preloadedRoom={payDialog.preloadedRoom}
      />
    </div>
  );
}
